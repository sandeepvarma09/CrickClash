...
async function runTests() {
  ...
  await test('GET /api/matches/seed creates mock matches', async () => {
    const { status, data } = await req('GET', '/matches/seed') as { status: number; data: { data: { seeded: number } } };
    assert(status === 200 || status === 403, `Expected 200/403, got ${status}`);
    if (status === 200) assert(typeof data.data?.seeded === 'number', 'No seeded count');
  });

  // Get matches
  let matchId = '';
  await test('GET /api/matches returns match list', async () => {
    const { status, data } = await req('GET', '/matches') as { status: number; data: { data: { items: { _id: string }[] } } };
    assert(status === 200, `Expected 200, got ${status}`);
    assert(Array.isArray(data.data?.items) || Array.isArray(data.data), 'No match array');
    const items = Array.isArray(data.data) ? data.data : data.data?.items;
    if (items?.[0]) matchId = items[0]._id;
  });
  ...
  await test('POST /api/challenges creates challenge', async () => {
    if (!matchId) { console.log('  ⚠️  Skipped: no matchId'); return; }
    ...
    }) as { status: number; data: { data: { challengeId: string } } };
  ...
  await test('GET /api/challenges/:id returns challenge', async () => {
    if (!challengeId) { console.log('  ⚠️  Skipped: no challengeId'); return; }
    const { status, data } = await req('GET', `/challenges/${challengeId}`) as { status: number; data: { data: { challenge: { challengeId: string } } } };
    assert(status === 200, `Expected 200, got ${status}`);
  ...
  await test('GET /api/leaderboard/global returns array', async () => {
    const { status, data } = await req('GET', '/leaderboard/global') as { status: number; data: { data: unknown[] } };
    assert(status === 200, `Expected 200, got ${status}`);
  ...
  else console.log('🎉 All API tests passed!\n');
}
